...
| `Attributes`    | Canonical Instance attribute-name constants (per-player `Checkpoint`, `InSafeRoom`, `KeyCount`, `GameState`, `FlashlightBattery`, `RespawnGrace`, `Stamina`, `FlashlightOn`, `NoiseLevel`, `Downed`, `DownedUntil`, `ReviveProgress`, `Hidden`, `FlareCharges`; client-local prefs `SettingFxIntensity`/`SettingReduceFlashing`/`SettingMasterVolume`; per-rig `Species`, `State`) — the `Tags` discipline applied to attributes. |
...
| `Remotes`       | RemoteEvent registry under one ReplicatedStorage folder; fetch by name. Server→client cues (incl. `MonsterStateChanged`, `TrapTriggered`, `ExitOpened`, `RoundThemeAnnounced`, `AmbushTriggered`, `AlarmStateChanged`), and the **client→server** remotes `SprintIntent` / `ShopAction` / `ThrowDecoy` / `ThrowFlare` / `HideIntent` (each validated + rate-limited server-side; payloads treated as hostile). |
...
| `EnvironmentService` | server | Rebuilds the horror MOOD from code on boot (so a fresh build isn't flat/bright/fogless and the place content stays reproducible): sets `Lighting.Technology` (= `ShadowMap` — deliberately NOT Future, which dims/breaks the flashlight `SpotLight` on iPad GPU tiers; the value is also pinned as place content in `default.project.json` `$properties` so a fresh build is deterministic and the script write is a redundant self-heal), the dark Ambient/exposure, and idempotently find-or-creates the `Atmosphere` haze + `Bloom` + `DepthOfField` + a base `ColorCorrection` ("BaseGrade") from `Config.Environment`. Never touches the client's "ChaseGrade". |
...
| `FlareService` | server | Server-authoritative throwable FLARE — light as a WEAPON (the dark-vs-light theme made a player VERB). On the `ThrowFlare` intent (no payload; rate-limited + per-round `Config.Flare.Charges` budget + cooldown) it computes the landing from the player's OWN known position+facing (never a client position), clamps it to the first wall, spawns a replicated burning flare, and FREEZES every rig within `Config.Flare.BlindRadius` by writing the existing `MonsterStunnedUntil` seam (the SAME one `TrapService` uses → `MonsterService` freezes the rig + skips the contact-catch, so **zero MonsterService change**). The universal "get it off me NOW" tool that works on BOTH monsters (the decoy only fools the near-blind monkey). Charges refill on `RoundReset`. `Config.Flare`. |
...
| `DownedService` | server | Co-op rescue: turns a catch into a revivable **DOWNED** state (immobilized + bleeding out) instead of an instant respawn. **Gated by `Config.Downed.Enabled` (default FALSE → a catch is a reliable instant checkpoint respawn; flip on for the co-op revive flow once playtested with 2+ players).** Reacts to `ServerSignals.PlayerCaught`: if enabled and a teammate could save them → `down` (sets `Downed`/`DownedUntil`, **ANCHORS the HRP** so it freezes in place — NOT PlatformStand, whose ragdoll dropped/clipped players through the floor — fires `MonsterCaught` with `downed=true`); else (solo / full wipe / disabled) → the classic kill → checkpoint respawn. Each poll it bleeds out (→ death respawn) or, if a living teammate STAYS within `Config.Downed.ReviveRadius` for `ReviveSeconds` (spatial, server-authoritative — no client input), **revives** them with a brief grace. A fresh character clears the state. Single-player is unchanged (no rescuer → kill). On the KILL path it fires `ServerSignals.PlayerKilled` (so `KeyService` can apply the gated drop-a-key stake). `Config.Downed`. |
| `HidingService` | server | The last-resort ESCAPE (the genre's lockers). On `HideIntent` (no payload, rate-limited) it spatially confirms the player is within `Config.Hiding.EnterRadius` of a `Tags.HidingSpot` from its OWN known position, teleports + **ANCHORS** them in (like `DownedService`, never PlatformStand), and sets the `Hidden` attribute — `MonsterService` then drops them from sensing AND contact-catch (and being still zeroes their noise), exactly like a SafeRoom. NOT god-mode: a HUNTING rig (Chase/Investigate) lingering within `DiscoverRadius` for `DiscoverSeconds` **FLUSHES** them out (+ brief `RespawnGrace`), so you must break sight BEFORE hiding. Clears on death/CharacterAdded/RoundReset. Inert until `HidingSpot` markers are placed. `Config.Hiding`. |
| `AmbushService` | server | Wires the (previously dead) `Tags.AmbushNode` into one-shot MICRO-SCARES between encounters: a spatial poll fires `AmbushTriggered` to the lobby when a player passes within `Config.Ambush.TriggerRadius` of a node (per-player-per-node cooldown), and the client renders a localized 3D skitter + a brief light cut. Skips InSafeRoom/Won/Downed. Decides nothing; inert until `AmbushNode` markers are placed. `Config.Ambush`. |
...
| `HUDController` | client | Icon-forward HUD: key pips (count = `Config.Keys.RequiredToWin`), battery + stamina gauges, an exit arrow shown only once all keys are in, transient feedback toasts (`KEY n/3`, `EXIT LOCKED`), and a win overlay; plus an **ALL-KEYS** green celebration flash + "RUN FOR THE EXIT" toast (when the team holds the last key), a low-battery gauge throb, and an exit-arrow pulse — all honoring the REDUCE-FLASHING comfort pref. Renders only. |
...
| `FlareController` | client | Input + HUD for the FLARE: binds `Config.Flare.ThrowKey`/gamepad + an on-screen **Flare** touch button to fire `ThrowFlare`, and renders a `FLARE xN` charge chip with a cooldown drain (dims at 0 charges). `FlareService` owns the throw + blind entirely; this only captures input + shows charges. `Config.Flare`. |
...
| `MonsterAudioController` | client | 3D breathing/footsteps on each rig, a distance-scaled heartbeat, and a Patrol→Chase sting (off `MonsterStateChanged`). The continuous heartbeat/tension beds are gated PER-TARGET (only a monster hunting YOU, via the per-target `MonsterStateChanged` remote — not the global rig `State`) and silenced in a SafeRoom, so a monster chasing a teammate doesn't pound your heart. |
...
| `SettingsController` | client | The comfort/ACCESSIBILITY menu (the FX-heavy game had none). A persistent top-left **gear button** (+ `Config.Settings.ToggleKey`/gamepad) opens FX-intensity + master-volume steppers and a REDUCE-FLASHING toggle, plus a HOW-TO-PLAY recall of the controls + red/yellow threat legend. Writes the prefs as client-local `Setting*` attributes the FX controllers consume (master volume sets the `AudioGroups` Master bus). Render-only, client-local (no server trust). `Config.Settings`. |
| `HideController` | client | Input + HUD for the hiding mechanic: binds `Config.Hiding.HideKey`/gamepad + an on-screen **Hide** touch button to fire `HideIntent`; shows a "press to HIDE" prompt near a `HidingSpot` and the `HIDDEN` overlay while the `Hidden` attribute is set. `HidingService` owns the actual hide; this only captures input + renders. `Config.Hiding`. |
| `AlarmFXController` | client | The endgame ALARM (off `AlarmStateChanged`): when the team holds every key, a pulsing red emergency wash + alarm sting + low bed until the round ends — the climax made felt. Honors the `Setting*` comfort prefs; its own ScreenGui/Sounds. `Config.Endgame`. |
| `AmbushFXController` | client | Renders `AmbushService`'s scares (off `AmbushTriggered`): a localized 3D skitter at the node + a brief cut of the nearest `FlickerLight`. Respects REDUCE-FLASHING. Render-only; `Config.Ambush`. |
