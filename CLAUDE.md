...
| `Attributes`    | Canonical Instance attribute-name constants (per-player `Checkpoint`, `InSafeRoom`, `KeyCount`, `GameState`, `FlashlightBattery`, `RespawnGrace`, `Stamina`, `FlashlightOn`, `NoiseLevel`, `Downed`, `DownedUntil`, `ReviveProgress`, `Hidden`; client-local prefs `SettingFxIntensity`/`SettingReduceFlashing`/`SettingMasterVolume`; per-rig `Species`, `State`) — the `Tags` discipline applied to attributes. |
