...
| `Remotes`       | RemoteEvent registry under one ReplicatedStorage folder; fetch by name. Server→client cues (incl. `MonsterStateChanged`, `TrapTriggered`, `ExitOpened`), and the one **client→server** remote `SprintIntent` (validated + rate-limited server-side). |
...
| `DownedService` | server | Co-op rescue: turns a catch into a revivable **DOWNED** state (collapsed + immobilized + bleeding out) instead of an instant respawn. Reacts to `ServerSignals.PlayerCaught`: if a teammate could save them → `down` (sets `Downed`/`DownedUntil`, PlatformStand, fires `MonsterCaught` with `downed=true`); else (solo / full wipe / disabled) → the classic kill → checkpoint respawn. Each poll it bleeds out (→ death respawn) or, if a living teammate STAYS within `Config.Downed.ReviveRadius` for `ReviveSeconds` (spatial, server-authoritative — no client input), **revives** them with a brief grace. A fresh character clears the state. Single-player is unchanged (no rescuer → kill). `Config.Downed`. |
